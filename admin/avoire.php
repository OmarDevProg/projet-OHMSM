...
<head>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const ticketInput = document.querySelector("input[name='ticket_id']");
            ...
            ticketInput.addEventListener("change", function() {
                ...
                if (ticketId) {
                    fetch(`get_produits_ticket.php?ticket_id=${ticketId}`)
                        .then(response => response.json())
                        .then(data => {
                            // Supprimer uniquement les produits existants sans toucher à l'input ticket_id
                            // container.querySelectorAll(".produit-ligne").forEach(el => el.remove());

                            if (data.length > 0) {
                                data.forEach(produit => {
                                    const produitDiv = document.createElement("div");
                                    produitDiv.classList.add("produit", "mb-3", "d-flex", "align-items-end", "produit-ligne");

                                    produitDiv.innerHTML = `
                    <div class="form-group me-2">
                        <input type="hidden" class="form-control" name="ticket_id" value="${produit.ticket_id}">
                        <label>Code-barres :</label>
                        <input type="text" class="form-control" name="code_barre[]" value="${produit.code_barre}" readonly>
                    </div>

                    <div class="form-group me-2">
                        <label>Produit :</label>
                        <input type="text" class="form-control" value="${produit.produit}" readonly>
                    </div>

                    <div class="form-group me-2">
                        <label>Quantité disponible :</label>
                        <input type="number" class="form-control" value="${produit.quantite}" readonly>
                    </div>

                    <div class="form-group me-2">
                        <label>Quantité à retourner :</label>
                        <input type="number" class="form-control" name="quantite[]" min="0" max="${produit.quantite}" required>
                    </div>
                `;
                                ...
                                toastr.error("Aucun produit trouvé pour ce ticket.");
                            }
                        })
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const container = document.getElementById("produitsContainer");
            const addButton = document.getElementById("ajouterProduit");

            addButton.addEventListener("click", function() {
                const original = document.querySelector(".produit-ligne");
                const clone = original.cloneNode(true);

                // Réinitialiser les champs clonés
                clone.querySelector("input[name='code_barre[]']").value = "";
                clone.querySelector("input[name='quantite[]']").value = "";

                // Afficher le bouton "X" pour supprimer une ligne
...
?>
            <li class="nav-item ">
                <a class="nav-link dropdown-toggle bg-gradient-success" href="avoire.php">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                ...
                <h6 class="font-weight-bolder text-white mb-0">Avoir ticket</h6>
            </nav>
        ...
        <div class="row">
            <div class="col-md-11 mt-4">
                <div class="card shadow-lg rounded-lg">
                    <div class="card-header bg-light-subtle text-white px-4 py-3">
                        <h6 class="mb-0">Retour des Produits :</h6>
                    </div>

                    <div class="card-body p-4">
                        <form action="traitement_avoir.php" method="POST" id="retourForm">
                            <div id="produitsContainer">
                                <div class="produit mb-3 d-flex align-items-end produit-ligne">
                                    <div class="form-group me-2 w-100">
                                        <label for="ticket_id" class="form-label">Numéro de Ticket :</label>
                                        <input type="number" class="form-control" name="ticket_id" required placeholder="Entrez le numéro du ticket">
                                    </div>
                                </div>
                            </div>

                            <button type="submit" class="btn btn-success px-5 py-2 mt-3">Valider le retour</button>
                        </form>
                    </div>
                </div>
...
<div class="fixed-plugin">
    ...
    <div class="card shadow-lg">
        ...
        <div class="card-body pt-sm-3 pt-0 overflow-auto">
            ...
            <a class="btn bg-gradient-dark w-100" href="https://www.dasinformatique.com/product/argon-dashboard">Free Download</a>
            <a class="btn btn-outline-dark w-100" href="https://www.dasinformatique.com/learning-lab/bootstrap/license/argon-dashboard">View documentation</a>
            <div class="w-100 text-center">
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u=https://www.dasinformatique.com/product/argon-dashboard" class="btn btn-dark mb-0 me-2" target="_blank">
