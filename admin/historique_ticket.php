    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-body" id="ticketModalContent">
                <!-- Le ticket sera chargé ici via AJAX -->
...
<aside fragment="navbar" class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link dropdown-toggle bg-gradient-success" href="historique_ticket.php">
...
<main class="main-content position-relative border-radius-lg ">
    ...
    <div class="mt-4">
        <div class="row align-items-center">
            <div class="col-md-4">
                <!-- Champ date + bouton pour exporter les tickets par date -->
                <input type="date" id="filterDate" class="form-control d-inline-block w-auto ms-3 me-2" />
            </div>
            <div class="col-md-8 text-end">
                <!-- Bouton pour exporter tous les tickets -->
                <button class="btn btn-success" onclick="exportAllTicketsToExcel()">Exporter Tout (avec total)</button>
                <button class="btn btn-primary" onclick="exportTicketsByDate()">Exporter par Date</button>
            </div>
        </div>
        <div class="mt-3">
            <h6 id="resultTotal" class="text-dark fw-bold"></h6>
        </div>
    </div>
    ...
    <div class="row">
                ...
                <div class="card-body p-4">
                    <table id="clientTable" class="table align-items-center mb-0">
                        ...
                        <tr>
                            ...
                            <th class="text-uppercase text-xs font-weight-bold ps-2 d-none">Produit</th>
                            <th class="text-uppercase text-xs font-weight-bold ps-2 d-none">Quantité</th>
                            <th class="text-uppercase text-xs font-weight-bold ps-2 d-none">Prix Unitaire (TND)</th>
                            <th class="text-uppercase text-xs font-weight-bold ps-2 d-none">Remise (%)</th>
                            <th class="text-uppercase text-xs font-weight-bold ps-2 d-none">Prix Total (TND)</th>

                            <th class="text-uppercase text-xs font-weight-bold ps-2">Action</th>
     ...
     ORDER BY t.date DESC "
                        ...
                        $affiches = []; // stocker les ID déjà affichés

                        foreach ($produits as $produit) {
                        $ticketId = $produit['id'];
                        $formattedDate = date('Y-m-d', strtotime($produit['date']));

                        // Vérifie si le ticket a déjà été affiché
                        if (!in_array($ticketId, $affiches)) {
                        echo "<tr id='row-" . htmlspecialchars($ticketId) . "'>
                <td>" . htmlspecialchars($ticketId) . "</td>
                <td>" . $formattedDate . "</td>
                <td>" . htmlspecialchars($produit['total']) . "</td>
                <td class='d-none'>" . htmlspecialchars($produit['produit']) . "</td>
                <td class='d-none'>" . htmlspecialchars($produit['quantite']) . "</td>
                <td class='d-none'>" . htmlspecialchars($produit['prix_unitaire']) . "</td>
                <td class='d-none'>" . htmlspecialchars($produit['remise']) . "</td>
                <td class='d-none'>" . htmlspecialchars($produit['prix_total']) . "</td>
                <td>
                    <button class='btn btn-sm btn-primary' onclick='printTicket(\"" . htmlspecialchars($ticketId) . "\")'>
                        👀 aperçu
                    </button>
                    <button class='btn btn-sm btn-danger' onclick='ticketdelite(\"" . htmlspecialchars($ticketId) . "\")'>
                        🗑️ Supprimer
                        </button>

                        </td>
                        </tr>";
                        $affiches[] = $ticketId; // ajoute à la liste
                        }
                        }
                        ?>
    ...
    <div class="row" hidden="hidden">
        <div class="col-md-12">
            <div class="card shadow-lg rounded-lg">
                ...
                <div class="card-body p-4">
                    <table id="clientTable1" class="table align-items-center mb-0">
                        <thead>
                        <tr>
                            <th class="text-uppercase text-xs font-weight-bold ps-2">ID Ticket</th>
                            <th class="text-uppercase text-xs font-weight-bold ps-2">Date</th>
                        ...
                        foreach ($produits as $produit) {
                            ...
                            echo "<tr id='row-" . htmlspecialchars($produit['id']) . "'>
                                    ...
                                    <td>" . htmlspecialchars($produit['prix_total']) . " </td>
                                     <td>
            <button class='btn btn-sm btn-primary' onclick='printTicket(\"" . htmlspecialchars($produit['id']) . "\")'>
                🖨️ Imprimer
            </button>
        </td>
    ...
    <script>
        function printTicket(ticketId) {
            fetch('print_ticket.php?id=' + ticketId, {
                headers: {
            ...
            "buttons": [
                {
                    ...
                    exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5, 6, 7] // Excludes the last column (Action)
                    }
                },
                {
                    extend: 'pdf',
...
<!--script datatable for hidden table -->

<script>
    $(document).ready(function() {
        // Initialize DataTable
        var table = $('#clientTable1').DataTable();
...
<script>
    function calculateTotal() {
        let total = 0;
        const rows = document.querySelectorAll("#ticketTableBody1 tr");

        rows.forEach(row => {
            const prixTotalCell = row.cells[7]; // 8th column = Prix Total (TND)
            const prix = parseFloat(prixTotalCell.textContent.trim()) || 0;
            total += prix;
        });

        document.getElementById("resultTotal").textContent = `Total Global: ${total.toFixed(3)} TND`;
    }

    function calculateTotalByDate() {
        ...
        rows.forEach(row => {
            ...
            if (dateCell === selectedDate) {
                const prix = parseFloat(row.cells[7].textContent.trim()) || 0;
                total += prix;
            }
        });

        document.getElementById("resultTotal").textContent = `Total pour ${selectedDate}: ${total.toFixed(3)} TND`;
    }
</script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
<script>
    function exportAllTicketsToExcel() {
        const table = $('#clientTable1').DataTable();
        const data = [["ID Ticket", "Date", "Total (TND)", "Produit", "Quantité", "Prix Unitaire (TND)", "Remise (%)", "Prix Total (TND)", "Montant Remise", "Marge (30%)", "Marge après Remise (30%)"]];

        let totalPrix = 0;
        let totalRemise = 0;
        let totalMarge = 0;
        let totalMargeApresRemise = 0;
        ...
        table.rows().every(function () {
            ...
            const quantite = parseFloat(row[4]) || 0; // Quantity
            const prixUnitaire = parseFloat(row[5]) || 0; // Unit Price
            const remisePercent = parseFloat(row[6]) || 0; // Discount %
            const prixTotal = parseFloat(row[7]) || 0; // Total Price

            const montantRemise = (prixUnitaire * remisePercent / 100) * quantite; // Discount Amount
            const marge = prixUnitaire * 0.3; // Margin (30%)
            const margeApresRemise = marge - montantRemise; // Margin After Discount

            // Push only relevant columns (exclude Action column)
            rowData.push(row[0]); // ID Ticket
            rowData.push(row[1]); // Date
            rowData.push(row[2]); // Total (TND)
            rowData.push(row[3]); // Produit
            rowData.push(row[4]); // Quantité
            rowData.push(row[5]); // Prix Unitaire (TND)
            rowData.push(row[6]); // Remise (%)
            rowData.push(row[7]); // Prix Total (TND)

            // Add calculated fields
            rowData.push(montantRemise.toFixed(3));
            rowData.push(marge.toFixed(3));
            rowData.push(margeApresRemise.toFixed(3));

            // Update totals
            totalPrix += prixTotal;
            totalRemise += montantRemise;
            totalMarge += marge;
            totalMargeApresRemise += margeApresRemise;

            data.push(rowData);
        });

        // Add totals row
        data.push(["", "", "", "", "", "", "TOTAL", totalPrix.toFixed(3), totalRemise.toFixed(3), totalMarge.toFixed(3), totalMargeApresRemise.toFixed(3)]);

        // Export to Excel
        const worksheet = XLSX.utils.aoa_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Tous les Tickets");
        XLSX.writeFile(workbook, "tous_les_tickets.xlsx");
    }

    function exportTicketsByDate() {
        const selectedDate = document.getElementById("filterDate").value;
        if (!selectedDate) {
            alert("Veuillez sélectionner une date.");
            return;
        ...
        table.rows().every(function () {
            const row = this.data();
            const rowDate = row[1].trim(); // Get the date from the row

            if (rowDate === selectedDate) {
                const rowData = [];

                // Extract and calculate values
                const quantite = parseFloat(row[4]) || 0;
                const prixUnitaire = parseFloat(row[5]) || 0;
                const remisePercent = parseFloat(row[6]) || 0;
                const prixTotal = parseFloat(row[7]) || 0;

                const montantRemise = (prixUnitaire * remisePercent / 100) * quantite;
                const marge = prixUnitaire * 0.3;
                const margeApresRemise = marge - montantRemise;

                // Push only relevant columns
                rowData.push(row[0]); // ID Ticket
                rowData.push(row[1]); // Date
                rowData.push(row[2]); // Total (TND)
                rowData.push(row[3]); // Produit
        ...
        if (data.length === 1) {
            alert("Aucun ticket trouvé pour cette date.");
            return;
        }
        ...
        const worksheet = XLSX.utils.aoa_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Tickets " + selectedDate);
        XLSX.writeFile(workbook, `tickets_${selectedDate}.xlsx`);
    }
</script>
<script>
    function ticketdelite(ticketId) {
        Swal.fire({
            title: 'Êtes-vous sûr ?',
            text: "Cette action supprimera le ticket définitivement !",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "delete_ticket.php?id=" + encodeURIComponent(ticketId);
            }
        });
    }
</script>
