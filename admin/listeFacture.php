...
<aside fragment="navbar" class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link  dropdown-toggle bg-gradient-success" data-bs-toggle="collapse" href="#submenuFacture" role="button" aria-expanded="false" aria-controls="submenuUtilisateur">
                    <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
                        <i class="fa-solid fa-users-gear" style="font-size: 14px; color: #4f4f4f;"></i>
                    </div>
                ...
                <div class="collapse" id="submenuFacture">
                    <ul class="nav flex-column ms-4">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="listeFacture.php">
                                <i class="bi bi-list-ul text-dark text-sm opacity-10 me-2"></i>
...
<main class="main-content position-relative border-radius-lg ">
        ...
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                    ...
                    <li class="breadcrumb-item text-sm text-white active" aria-current="page">Gestion des factures</li>
                </ol>
                <h6 class="font-weight-bolder text-white mb-0">Liste des factures</h6>
            </nav>
    ...
    $stmt_articles = $db->prepare("SELECT fa.*, p.*,c.*,fat.* FROM produits p , facture fa,client c,facture_articles fat  where c.id=fa.client_id and fat.facture_id=fa.id and fat.produit_id=p.id");
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-md-12">
                <div class="card shadow-lg rounded-lg">
                    ...
                    <div class="card-body p-4">    <table id="clientTable" class="table align-items-center mb-0">
        <thead>
        <tr>
            <th class="text-uppercase text-xs font-weight-bold ps-2">Nom de client</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">prenom</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">matricule</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">date de creation</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">designation</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">quantité</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">prix de produit</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">prix HT</th>
            <th class="text-uppercase text-xs font-weight-bold ps-2">prix TTC</th>
        ...
        <?php foreach ($articles as $article): ?>
            <tr>
                ...
                <td><?php echo htmlspecialchars($article['prenom']); ?></td>
                <td><?php echo htmlspecialchars($article['matricule']); ?></td>
                ...
                <td><?php echo htmlspecialchars($article['prix_total_ht']); ?></td>
                <td><?php echo htmlspecialchars($article['prix_total_ttc']); ?></td>



                <td>
                    <a href="javascript:void(0);" class="btn btn-danger btn-sm"
                       onclick="confirmDeleteFacture(<?php echo $article['facture_id']; ?>)">
    ...
    <script src="https://cdn.jsdelivr.net/npm/datatables.net-bs4s/js/dataTables.bootstrap4.min.js"></script>
    ...
    <script>
        ...
        // Confirm delete action for facture
        function confirmDeleteFacture(factureId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Redirect to delete facture action
                    window.location.href = 'deleteFacture.php?id=' + factureId;
                }
            });
        }
    </script>
