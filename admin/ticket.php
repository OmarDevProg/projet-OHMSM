...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($codesBarres as $i => $codeBarre) {
        ...
        if ($produit) {
            ...
            $quantite = isset($quantites[$i]) ? intval($quantites[$i]) : 0;
            $remise = isset($remises[$i]) ? floatval($remises[$i]) : 0; // Ensure remise is always processed
            ...
            $prixTotal = ($prixUnitaire * $quantite) - (($remise / 100) * $prixUnitaire * $quantite);
            $totalPrix += $prixTotal;
            ...
            $ticketDetails[] = [
                ...
                'remise' => $remise, // Store remise even if it's 0
